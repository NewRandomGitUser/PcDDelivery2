...
long long finalCellsUsingReductionTime = 0;
long long finalCellsUsingCriticalTime = 0;
...
int finalAlivedCellsUsingCritical(int **grid, int nThreads) {
    struct timeval inicioFinalAlivedCells, finalFinalAlivedCells;
    long long tmili;
    gettimeofday(&inicioFinalAlivedCells, NULL);
    int total = 0;
    omp_set_num_threads(nThreads);
    #pragma opm paralell for shared(total)
    {
        for(int i = 0; i<N; i++){
            for(int j=0; j<N; j++)
            {
    ...
    gettimeofday(&finalFinalAlivedCells, NULL);
    tmili = (int) (1000*(finalFinalAlivedCells.tv_sec - inicioFinalAlivedCells.tv_sec) + 
    (finalFinalAlivedCells.tv_usec - inicioFinalAlivedCells.tv_usec) / 1000);
    finalCellsUsingCriticalTime = tmili;
    return total;
}

int finalAlivedCellsUsingReduction(int **grid, int nThreads) {
    struct timeval inicioFinalAlivedCells, finalFinalAlivedCells;
    long long tmili;
    gettimeofday(&inicioFinalAlivedCells, NULL);
    int total = 0;
    ...
    tmili = (int) (1000*(finalFinalAlivedCells.tv_sec - inicioFinalAlivedCells.tv_sec) + 
    (finalFinalAlivedCells.tv_usec - inicioFinalAlivedCells.tv_usec) / 1000);
    finalCellsUsingReductionTime = tmili;
    return total;
}
...
void loopGenerations(int **grid, int **newGrid, int numberOfGenerations, int nThreads) {
    paralellCodeTime = 0;
    finalCellsUsingReductionTime = 0;
    finalCellsUsingCriticalTime = 0;
...
long long ExecuteGameOfLifeWithNThreads(int nThreads) {
    ...
    finalAlivedCellsUsingReduction(grid, nThreads);
    finalAlivedCellsUsingCritical(grid, nThreads);
...
int main() {
    ...
    long long timeVariationFinalCellsUsingReduction[4] = {0,0,0,0};
    long long timeVariationFinalCellsUsingCritical[4] = {0,0,0,0};
    ...
    for(int i=0; i<4; i++) {
        ...
        timeVariationFinalCellsUsingReduction[i] = finalCellsUsingReductionTime;
        timeVariationFinalCellsUsingCritical[i] = finalCellsUsingCriticalTime;
    }
    for(int i=0; i<4; i++) {
        printf("Execution for [%d] threads, Total Time: [%lldms],Paralell Code Variation Time: [%lldms], FinalCellsUsingReduction Variation Time: [%lldms],FinalCellsUsingCritical Code Variation Time: [%lldms],\n", 
        numberOfThreadsPerExecution[i], 
        totalTimeOfExecution[i],
        timeVariationParalellCodePerExecution[i],
        timeVariationFinalCellsUsingReduction[i],
        timeVariationFinalCellsUsingCritical[i]);
    }
    return 0;
}
